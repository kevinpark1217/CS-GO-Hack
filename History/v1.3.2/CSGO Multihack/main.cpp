		...
		for (int i = 1; i < 64; i++) {
			if (!EntityList.IsDormant(i) && !EntityList.IsDead(i) && EntityList.GetTeam(i) != NewPlayer.GetTeam() ) {
				...
				if (isRage && !isTrigger) {
					...
					if (fabs(boneScreen[0] - currentAng[0]) < 1.5f && fabs(boneScreen[1] - currentAng[1]) < 1.5f && NewPlayer.GetShotsFired() > 0 && bsp.Visible(myWorld, boneWorld)) {
						float val0 = (boneScreen[0] - currentAng[0])/3, val1 = (boneScreen[1] - currentAng[1])/3;
						/*for (int i = 0;i < 5; i++) {
							currentAng[0] += val0;
							currentAng[1] += val1;
						...
						int aim = 2;
						while (/*((GetAsyncKeyState(VK_XBUTTON1) & 0x8000) || (GetAsyncKeyState(VK_MBUTTON) & 0x8000)) && */!EntityList.IsDead(i) && !EntityList.IsDormant(i) && NewPlayer.GetShotsFired() > 1 && bsp.Visible(myWorld, boneWorld)) {
