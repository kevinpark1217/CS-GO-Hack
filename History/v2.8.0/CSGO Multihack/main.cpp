...
bool isRunning, panelEnabled, isAimbotFix;
...
void calibrate() {
	...
	aimSensX = 50 / zeroAng[1];
	aimSensY = 50 / zeroAng[0];
	...
	NewPlayer.GetAngles(zeroAng);
	aimSensScopeX = 50 / zeroAng[1];
	aimSensScopeY = 50 / zeroAng[0];
	std::cout << "\nFinished!" << std::endl;
}
...
int main() {
	...
	while (!isPanic) {
		...
		if (inGame == 6 && !NewPlayer.IsDead() && !isRunning) {
			loadMap();
			isRunning = true;
		} else if ((inGame != 6 || NewPlayer.IsDead()) && isRunning) {
			isRunning = false;
			activeMode = -1;
			Sleep(1000);
		}
...
void v1() {
	...
	while (true) {
		if (isPanic)
			...
			if (isRcs[activeMode] && NewPlayer.GetShotsFired() > 1)
			{
				float MyPunch[3];
			...
			for (loop = 0; loop < maxPlayer; loop++) {
				...
				if (fabs(boneScreen[0] - currentAng[0]) < smallest[0] && fabs(boneScreen[1] - currentAng[1]) < smallest[1] && (isAimWall || bsp.Visible(myWorld, boneWorld))) {
					smallest[0] = fabs(boneScreen[0] - currentAng[0]);
					smallest[1] = fabs(boneScreen[1] - currentAng[1]);
					i = loop;
				}
			}
			if (i != -1 && NewPlayer.GetWeaponClip() > 0 && ((isHoldAim[activeMode] && GetAsyncKeyState(aimbotHoldKey) & 0x8000) || NewPlayer.GetShotsFired() > 0)) {
				...
				int aimX = (int)(fabs(boneScreen[1] - currentAng[1]) / 16 * aimSmooth[activeMode]) + 1;
				int aimY = (int)(fabs(boneScreen[0] - currentAng[0]) / 16 * aimSmooth[activeMode]) + 1;
				int aim;
				if (aimX > aimY)
					aim = aimX;
				else
					aim = aimY;
				float val0 = (boneScreen[0] - currentAng[0]) / (aim), val1 = (boneScreen[1] - currentAng[1]) / (aim);
				...
				do {
					...
					if (isAimLegit) {
						...
						if (NewPlayer.IsScoped()) {
							sensX = fabs(boneScreen[1] - currentAng[1]) * aimSensScopeX;
							sensY = (boneScreen[0] - currentAng[0]) * aimSensScopeY;
						} else {
							sensX = fabs(boneScreen[1] - currentAng[1]) * aimSensX;
							sensY = (boneScreen[0] - currentAng[0]) * aimSensY;
						}
						if (sensX > 10.0f)
							continue;
				...
				} while (!NewPlayer.IsDead() && fabs(boneScreen[0] - currentAng[0]) < aimHeight / 10.f && fabs(boneScreen[1] - currentAng[1]) < aimWidth / 10.f &&!EntityList.IsDead(i) && !EntityList.IsDormant(i) && (NewPlayer.GetWeaponClip() > 0 && ((isHoldAim[activeMode] && GetAsyncKeyState(aimbotHoldKey) & 0x8000) || NewPlayer.GetShotsFired() > 0) && (isAimWall || bsp.Visible(myWorld, boneWorld))));
			}
		}
		Sleep(V1SLEEP + 20);
	}
