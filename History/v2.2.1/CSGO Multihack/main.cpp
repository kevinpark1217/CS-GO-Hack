	...
	for (int j = 0;j < 2;j++) {
		for (int i = 0;i < 4;i++) {
			...
			if (i == 0 && j == 0) {
				...
				glCallLists(14, GL_UNSIGNED_BYTE, "Smooth Factor");
				...
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 + 29 && p.x < x + xx*i + 150 + 34 && p.y > y + yy*j + 289 && p.y < y + yy*j + 295 && (GetKeyState(VK_LBUTTON) & 0x100) != 0)
				{
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150 + 29, y + yy*j + 289, 5, 6);
					if (!isExecuted[j][i] && infoY < 999) {
						isExecuted[j][i] = true;
						aimSmooth++;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 + 29 && p.x < x + xx*i + 150 + 34 && p.y > y + yy*j + 296 && p.y < y + yy*j + 302 && (GetKeyState(VK_LBUTTON) & 0x100) != 0)
				{
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150 + 29, y + yy*j + 296, 5, 6);
				...
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 288, 29, 14); //Aim Smooth
				snprintf(buf, 32, "%3d", aimSmooth);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 300);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150 + 29, y + yy*j + 288, 6, 7); //Delay Up
				DrawPanelBox(x + xx*i + 150 + 29, y + yy*j + 295, 6, 7); //Delay Down
			}
			else if (i == 3 && j == 0) {
				glRasterPos2f(x + xx*i + 30, y + yy*j + 100);
				glCallLists(8, GL_UNSIGNED_BYTE, "Enable");
				...
				glRasterPos2f(x + xx*i + 152, y + yy*j + 175);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 188, 29, 14); //Bhop Jump Key
				...
				glCallLists(8, GL_UNSIGNED_BYTE, "Chicken");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 375);
				glCallLists(5, GL_UNSIGNED_BYTE, "Bomb");
				...
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 338 && p.y < y + yy*j + 352 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isChicken = !isChicken;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 363 && p.y < y + yy*j + 373 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isBomb = !isBomb;
					}
				...
				DrawPanelBox(x + xx*i + 150, y + yy*j + 338, 14, 14); //Chicken
				if (isChicken)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 340, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 363, 14, 14); //Bomb
				if (isBomb)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 365, 11, 11);
			}
			else if (i == 1 && j == 1) {
				glRasterPos2f(x + xx*i + 30, y + yy*j + 100);
			...
			if (!EntityList.IsDormant(i) && !EntityList.IsDead(i)) {
				...
				if (isRage && !isTrigger && !isAimbot) {
					if (bsp.Visible(myWorld, boneWorld) && EntityList.GetTeam(i) != 0 && ( imT != enT || isRageFriendly) && !EntityList.IsDormant(i) && !EntityList.IsDead(i)) {
						if (isRageSpin) {
							int loopCount = (int)((boneScreen[1] - currentAng[1]) / rageSpeed);
							float ychange = (boneScreen[0] - currentAng[0]) / loopCount;
							for (int i = 0;i < loopCount; i++) {
								currentAng[1] += rageSpeed;
								currentAng[0] += ychange;
								NewPlayer.SetAngles(currentAng);
								Sleep(5);
							}
							EntityList.GetBonePosition(i, boneWorld, 6);
							NewPlayer.GetPosition(myWorld);
					...
					if (fabs(boneScreen[0] - currentAng[0]) < 2.0f && fabs(boneScreen[1] - currentAng[1]) < 1.0f && (isAimWall || bsp.Visible(myWorld, boneWorld)) && ((isAimbotFriendly && NewPlayer.GetTeam() == EntityList.GetTeam(i) && imT == enT) || (imT != enT && NewPlayer.GetTeam() != EntityList.GetTeam(i)))) {
						...
						int aim = aimSmooth;
						while (fabs(boneScreen[0] - currentAng[0]) < 10.0f && fabs(boneScreen[1] - currentAng[1]) < 10.0f &&!EntityList.IsDead(i) && !EntityList.IsDormant(i) && ((isLegitAim && NewPlayer.GetShotsFired() > 0 && NewPlayer.GetWeaponClip() > 0) || (!isLegitAim && GetAsyncKeyState(aimbotHoldKey) & 0x8000)) && (isAimWall || bsp.Visible(myWorld, boneWorld))) {
				...
				for (int i = 0; i < objCount; i++) {
					...
					if (glowObj.pEntity != NULL) {
						...
						if (strstr(chModel, "ied") && isBomb) {
							...
							if (!isFriendly && EntityList.GetTeam(glowObj.pEntity) == NewPlayer.GetTeam()) {
								glowObj.r = 0;
								glowObj.g = 0;
								glowObj.b = 255;
							}
