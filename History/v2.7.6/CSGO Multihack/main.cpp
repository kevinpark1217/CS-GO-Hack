	...
	while (true) {
		if (isPanic)
			...
			if (i != -1 && NewPlayer.GetWeaponClip() > 0 && ((isHoldAim[activeMode] && GetAsyncKeyState(aimbotHoldKey) & 0x8000) || NewPlayer.GetShotsFired() > 0)) {
				...
				int aimX = (int)(fabs(boneScreen[1] - currentAng[1]) / 16 * aimSmooth[activeMode]);
				int aimY = (int)(fabs(boneScreen[0] - currentAng[0]) / 16 * aimSmooth[activeMode]);
				int aim;
				if (aimX > aimY)
					aim = aimX;
				else
					aim = aimY;
				float val0 = (boneScreen[0] - currentAng[0]) / (aim + 1), val1 = (boneScreen[1] - currentAng[1]) / (aim + 1);
				...
				do {
					...
					if (isAimLegit) {
						float sensX = fabs(boneScreen[1] - currentAng[1]) * aimSensX, sensY = (boneScreen[0] - currentAng[0]) * aimSensY;
						if (sensX > 10.0f)
							continue;
						if (sensX < 0.01f)
							sensX = 0.01f;
				...
				} while (fabs(boneScreen[0] - currentAng[0]) < 20.0f && fabs(boneScreen[1] - currentAng[1]) < 10.0f &&!EntityList.IsDead(i) && !EntityList.IsDormant(i) && (NewPlayer.GetWeaponClip() > 0 && ((isHoldAim[activeMode] && GetAsyncKeyState(aimbotHoldKey) & 0x8000) || NewPlayer.GetShotsFired() > 0) && (isAimWall || bsp.Visible(myWorld, boneWorld))));
			}
		}
		Sleep(V1SLEEP + 20);
	}
