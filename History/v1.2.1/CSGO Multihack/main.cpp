...
DWORD DwLocalPlayer, DwEntityList, DwEnginePointer, DwViewAngle, DwGlow, DwViewMatrix = 0x04A16AE4, DwClientState = 0x005D0224, DwRadarBase = 0x04A561FC,  DwTraceEngine;
...
bool login() {
	...
	if (option == '1') {
		if (flag == '0') {
			...
			if (strcmp(currentVers, version))
				std::cout << "Update v" << currentVers << " is available!\nAt tekhak.com/csgo/tekhak.zip\n" << std::endl;
			return true;
		}
		else if (flag == '2') {
		...
		for (i = 1; i < 64; i++) {
			if (EntityList.GetTeam(i) != NewPlayer.GetTeam() && !EntityList.IsDead(i) && !EntityList.IsDormant(i)) {
				...
				if (fabs(boneScreen[0] - currentAng[0]) < 1.5f && fabs(boneScreen[1] - currentAng[1]) < 1.5f) {
					while (((GetAsyncKeyState(VK_XBUTTON1) & 0x8000) || (GetAsyncKeyState(VK_MBUTTON) & 0x8000)) && !EntityList.IsDead(i) && !EntityList.IsDormant(i)) {
						EntityList.GetBonePosition(i, boneWorld, 10);
						NewPlayer.GetPosition(myWorld);
						myWorld[2] += NewPlayer.GetViewOrigin();
						worldToAngle(boneWorld, myWorld, boneScreen);
...
void rage() {
	...
	while (true) {
		...
		for (int i = 1; i < 64; i++) {
			if (EntityList.GetTeam(i) != NewPlayer.GetTeam() && !EntityList.IsDead(i) && !EntityList.IsDormant(i) && EntityList.isSpotted(i)) {
				EntityList.GetBonePosition(i, boneWorld, 10);
				NewPlayer.GetPosition(myWorld);
				myWorld[2] += NewPlayer.GetViewOrigin();
				worldToAngle(boneWorld, myWorld, boneScreen);
				...
				NewPlayer.SetAngles(boneScreen);
				Sleep(10);
				while (Mem.Read<DWORD>(modClient.dwBase + DwEntityList + (NewPlayer.GetCrosshairId() * 0x10)) == EntityList.GetBaseEntity(i)) {
					if (!EntityList.IsDead(i) && !EntityList.IsDormant(i)) {
						click();
						EntityList.GetBonePosition(i, boneWorld, 10);
						NewPlayer.GetPosition(myWorld);
						myWorld[2] += NewPlayer.GetViewOrigin();
