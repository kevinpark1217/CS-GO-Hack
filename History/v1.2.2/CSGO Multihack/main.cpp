	...
	if (option == '1') {
		if (flag == '0') {
			...
			if (strcmp(currentVers, version)) {
				...
				char toUpdate = 0;
				std::cout << "\nUpdate v" << currentVers << " is available!\nWould you like to update?" << std::endl;
...
void v1() {
	...
	while (true) {
		...
		for (int i = 1; i < 64; i++) {
			if (!EntityList.IsDormant(i) && !EntityList.IsDead(i) && EntityList.GetTeam(i) != NewPlayer.GetTeam() && EntityList.isSpotted(i)) {
				...
				if (isRage && !isTrigger) {
					...
					Sleep(5);
					while (Mem.Read<DWORD>(modClient.dwBase + DwEntityList + (NewPlayer.GetCrosshairId() * 0x10)) == EntityList.GetBaseEntity(i) && !EntityList.IsDormant(i) && !EntityList.IsDead(i)) {
						click();
						EntityList.GetBonePosition(i, boneWorld, 10);
						NewPlayer.GetPosition(myWorld);
						myWorld[2] += NewPlayer.GetViewOrigin();
