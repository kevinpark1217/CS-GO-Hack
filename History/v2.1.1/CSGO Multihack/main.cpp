						...
						for (int j = 1; j < 64; j++) {
							if (glowObj.pEntity == EntityList.GetBaseEntity(j) && glowObj.pEntity != NewPlayer.GetDwLocalPlayer()) {
								...
								if (!EntityList.IsDead(j) && !EntityList.IsDormant(j)) {
									...
									int color[4];
									if (!isFriendly && EntityList.GetTeam(j) == NewPlayer.GetTeam()) {
										color[0] = 0;
										color[1] = 0;
										color[2] = 255;
									}
		...
		for (int i = 1; i < 64; i++) {
			...
			if (!EntityList.IsDormant(i) && !EntityList.IsDead(i)) {
				...
				if (isRage && !isTrigger && !isAimbot) {
					if (bsp.Visible(myWorld, boneWorld) && EntityList.GetTeam(i) != 0 && (EntityList.GetTeam(i) != NewPlayer.GetTeam() || isRageFriendly) && !EntityList.IsDormant(i) && !EntityList.IsDead(i)) {
						if (isRageSpin) {
							float angle[3];
					...
					if (fabs(boneScreen[0] - currentAng[0]) < 2.0f && fabs(boneScreen[1] - currentAng[1]) < 1.0f  && EntityList.GetTeam(i) != 0 && (isAimWall || bsp.Visible(myWorld, boneWorld)) && (isAimbotFriendly || EntityList.GetTeam(i) != NewPlayer.GetTeam())) {
