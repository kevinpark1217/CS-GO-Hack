		...
		if (isPanic)
			...
			for (int i = 0; i < maxPlayer; i++) {
				if (EntityList.GetBaseEntity(i) == 0x0)
					continue;
				if (!EntityList.IsDormant(i) && !EntityList.IsDead(i) && ((isAimbotFriendly && NewPlayer.GetTeam() == EntityList.GetTeam(i)) || (NewPlayer.GetTeam() != EntityList.GetTeam(i))) && NewPlayer.GetWeaponClip() > 0 && ((isHoldAim[activeMode] && GetAsyncKeyState(aimbotHoldKey) & 0x8000) || NewPlayer.GetShotsFired() > 0)) {
					float boneScreen[2];
					float boneWorld[3];
					float myWorld[3];
					float currentAng[3];
					...
					if (fabs(boneScreen[0] - currentAng[0]) < 2.0f && fabs(boneScreen[1] - currentAng[1]) < 1.0f  && (isAimWall || bsp.Visible(myWorld, boneWorld)) ) {
						...
						do {
							...
							Sleep(10);
							if ((aim == 0 || fabs(boneScreen[1] - currentAng[1]) < 0.01f && fabs(boneScreen[0] - currentAng[0]) < 0.01f) && isAimShoot[activeMode] && !isTrigger[activeMode]) {
								click();
								aim = -1;
							}
