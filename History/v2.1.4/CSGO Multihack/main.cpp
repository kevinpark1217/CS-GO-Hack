									...
									if (isFriendly || EntityList.GetTeam(j) != NewPlayer.GetTeam()) {
										...
										if (WorldToScreen(EnemyPos, EnemyXY))
										{
											...
											if (isHealth) {
												...
												float HealthHeight = ((boxPos[3] + HealthBarHeight - 1.0f) - (boxPos[1] - HealthBarHeight + 1.0f))*((100.0f - EntityList.GetHealth(j)) / 100.0f);
												DrawBox(boxPos[2] + HealthBarWidth, boxPos[3] + HealthBarHeight, boxPos[2] + HealthBarWidth*2, boxPos[1] - HealthBarHeight, BoxColor);
												DrawFilledBox(boxPos[2] + HealthBarWidth + 1.0f, boxPos[3] + HealthBarHeight - 1.0f, boxPos[2] + HealthBarWidth * 2 - 1.0f, boxPos[1] - HealthBarHeight + 1.0f, HealthBarBackColor);
												DrawFilledBox(boxPos[2] + HealthBarWidth + 1.0f, boxPos[3] + HealthBarHeight - 1.0f, boxPos[2] + HealthBarWidth * 2 - 1.0f, (boxPos[1] - HealthBarHeight + 1.0f) + (HealthHeight), HealthBarColor);
											}
...
void v1() {
	...
	while (true) {
		...
		for (int i = 0; i < maxPlayer; i++) {
			if (i > 0 && EntityList.GetBaseEntity(i) == EntityList.GetBaseEntity(i - 1))
				continue;
			float boneScreen[2];
			float boneWorld[3];
			float myWorld[3];
			if (!EntityList.IsDormant(i) && !EntityList.IsDead(i)) {
				...
				if (isRage && !isTrigger && !isAimbot) {
					...
					if (fabs(boneScreen[0] - currentAng[0]) < 2.0f && fabs(boneScreen[1] - currentAng[1]) < 1.0f && (isAimWall || bsp.Visible(myWorld, boneWorld)) && ((isAimbotFriendly && NewPlayer.GetTeam() == EntityList.GetTeam(i) && imT == enT) || (imT != enT && NewPlayer.GetTeam() != EntityList.GetTeam(i)))) {
						...
						int aim = 4;
						while (fabs(boneScreen[0] - currentAng[0]) < 10.0f && fabs(boneScreen[1] - currentAng[1]) < 10.0f &&!EntityList.IsDead(i) && !EntityList.IsDormant(i) && (( isLegitAim && NewPlayer.GetShotsFired() > 0) || GetAsyncKeyState(aimbotHoldKey) & 0x8000) && (isAimWall || bsp.Visible(myWorld, boneWorld))) {
