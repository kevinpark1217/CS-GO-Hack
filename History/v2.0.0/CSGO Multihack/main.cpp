...
void DrawPanel() {
	...
	if (GetCursorPos(&p))
	{
		...
		glVertex2f(p.x, p.y);
		glVertex2f(p.x+20, p.y+10);
		glVertex2f(p.x +10, p.y + 20);
		glEnd();
	}*/
	//DrawBox();
}
							...
							if (glowObj.pEntity == EntityList.GetBaseEntity(j)) {
								...
								if (!EntityList.IsDead(j) && !EntityList.IsDormant(j)) {
									...
									if (isFriendly || EntityList.GetTeam(j) != NewPlayer.GetTeam()) {
										...
										if (WorldToScreen(EnemyPos, EnemyXY) && distance > 10.0f)
										{
											...
											if (isHealth) {
												...
												DrawFilledBox(boxPos[2], boxPos[3], boxPos[2] + HealthBarWidth, (boxPos[3]) - (boxPos[3] - boxPos[1]) * (EntityList.GetHealth(j) / 100.0F), HealthBarColor);
											}
		...
		for (int i = 1; i < 64; i++) {
			if (!EntityList.IsDormant(i) && !EntityList.IsDead(i)) {
				...
				if (isRage && !isTrigger && !isAimbot) {
					if (bsp.Visible(myWorld, boneWorld) && (isRageFriendly || EntityList.GetTeam(i) != NewPlayer.GetTeam())) {
						if (isRageSpin) {
							float angle[3];
					...
					if (fabs(boneScreen[0] - currentAng[0]) < 2.0f && fabs(boneScreen[1] - currentAng[1]) < 1.0f && (isAimWall || bsp.Visible(myWorld, boneWorld)) && (isAimbotFriendly || EntityList.GetTeam(i) != NewPlayer.GetTeam())) {
