...
int aimbotToggleKey, aimBone[4], aimbotHoldKey, aimSmooth[4];
		...
		for (int i = 0;i < 3;i++) {
			...
			if (i == 0 && j == 0) {
				...
				if(!isHoldAim[editMode])
					panelBoxColor = 0.75f;
				DoCheckNumberBox(&isHoldAim[editMode], &aimbotHoldKey, "Hold Key", i, j, 6);
						...
						for (int j = 0; j < maxPlayer; j++) {
							...
							if (glowObj.pEntity == EntityList.GetBaseEntity(j) && glowObj.pEntity != NewPlayer.GetDwLocalPlayer() && !EntityList.IsDead(j) && !EntityList.IsDormant(j)) {
								...
								for (int b = 0; b < boneCount; b++)
								{
									...
									if (fabs(playerPosition[0] - bone3D[b][0]) > 90 || fabs(playerPosition[1] - bone3D[b][1]) > 90 || fabs(playerPosition[2] - bone3D[b][2]) > 90 || !WorldToScreen(bone3D[b], bone2D[b])) {
										bone2D[b][0] = -1;
										bone2D[b][1] = -1;
										continue;
									}
									...
									if (isWallExternal && bone2D[b][0] >= 0.0f && bone2D[b][1] >= 0.0f)
				...
				if (!EntityList.IsDormant(i) && !EntityList.IsDead(i)) {
					...
					if (isRage && !isTrigger[activeMode] && !isAimbot[activeMode]) {
						...
						if (fabs(boneScreen[0] - currentAng[0]) < 2.0f && fabs(boneScreen[1] - currentAng[1]) < 1.0f && (isAimWall || bsp.Visible(myWorld, boneWorld)) && ((isAimbotFriendly && NewPlayer.GetTeam() == EntityList.GetTeam(i) && imT == enT) || (imT != enT && NewPlayer.GetTeam() != EntityList.GetTeam(i)))) {
							...
							while (fabs(boneScreen[0] - currentAng[0]) < 10.0f && fabs(boneScreen[1] - currentAng[1]) < 10.0f &&!EntityList.IsDead(i) && !EntityList.IsDormant(i) && (((isHoldAim[activeMode] && GetAsyncKeyState(aimbotHoldKey) & 0x8000)) || (NewPlayer.GetShotsFired() > 0 && NewPlayer.GetWeaponClip() > 0)) && (isAimWall || bsp.Visible(myWorld, boneWorld))) {
