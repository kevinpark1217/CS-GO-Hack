								...
								if (!EntityList.IsDead(j) && !EntityList.IsDormant(j)) {
									...
									for (int p = 0;p < 4;p++) {
										if (p >= 2)
											...
											boxPos[j][p] = std::numeric_limits<float>::max();
									}
									...
									for (int b = 0; b < boneCount; b++)
									{
										...
										if (bone2D[b][0] < boxPos[j][0] && bone2D[b][0]>0.0f)
											boxPos[j][0] = bone2D[b][0];
										else if (bone2D[parentId][0] < boxPos[j][0] && bone2D[parentId][0]>0.0f)
											boxPos[j][0] = bone2D[parentId][0];

										if (bone2D[b][0] > boxPos[j][2])
											boxPos[j][2] = bone2D[b][0];
										else if (bone2D[parentId][0] > boxPos[j][2])
											boxPos[j][2] = bone2D[parentId][0];

										if (bone2D[b][1] < boxPos[j][1] && bone2D[b][1]>0.0f)
											boxPos[j][1] = bone2D[b][1];
										else if (bone2D[parentId][1] < boxPos[j][1] && bone2D[parentId][1]>0.0f)
											boxPos[j][1] = bone2D[parentId][1];

										if (bone2D[b][1] > boxPos[j][3])
											boxPos[j][3] = bone2D[b][1];
										else if (bone2D[parentId][1] > boxPos[j][3])
											boxPos[j][3] = bone2D[parentId][1];
									}
									if (isFriendly || EntityList.GetTeam(j) != NewPlayer.GetTeam()) {
										float PlayerPos[3];
										...
										if (WorldToScreen(EnemyPos, EnemyXY))
										{
											...
											float HealthBarWidth = (boxPos[j][2] - boxPos[j][0]) / 5;
											float HealthBarHeight = (boxPos[j][3] - boxPos[j][1]) / 5;
											if (isBox)
												DrawBox(boxPos[j][0] - HealthBarWidth, boxPos[j][1] - HealthBarHeight, boxPos[j][2] + HealthBarWidth, boxPos[j][3] + HealthBarHeight, BoxColor);
											if (isHealth) {
												int HealthBarBackColor[3] = { 254, 0, 0 };
												int HealthBarColor[3] = { 0, 254, 0 };
												float HealthHeight = ((boxPos[j][3] + HealthBarHeight - 1.0f) - (boxPos[j][1] - HealthBarHeight + 1.0f))*((100.0f - EntityList.GetHealth(j)) / 100.0f);
												DrawBox(boxPos[j][2] + HealthBarWidth, boxPos[j][3] + HealthBarHeight, boxPos[j][2] + HealthBarWidth*2, boxPos[j][1] - HealthBarHeight, BoxColor);
												DrawFilledBox(boxPos[j][2] + HealthBarWidth + 1.0f, boxPos[j][3] + HealthBarHeight - 1.0f, boxPos[j][2] + HealthBarWidth * 2 - 1.0f, boxPos[j][1] - HealthBarHeight + 1.0f, HealthBarBackColor);
												DrawFilledBox(boxPos[j][2] + HealthBarWidth + 1.0f, boxPos[j][3] + HealthBarHeight - 1.0f, boxPos[j][2] + HealthBarWidth * 2 - 1.0f, (boxPos[j][1] - HealthBarHeight + 1.0f) + (HealthHeight), HealthBarColor);
											}
											...
											if (isName) {
												...
												glColor3f(204, 204, 0);
												glRasterPos2f(boxPos[j][0] - HealthBarWidth, boxPos[j][1] - HealthBarHeight - 4);
											...
											if (isWeapon) {
												...
												glColor3f(204, 204, 0);
												glRasterPos2f(boxPos[j][0] - HealthBarWidth, boxPos[j][3] + HealthBarHeight + 12);
		...
		for (int i = 0; i < 64; i++) {
			...
			if (!EntityList.IsDormant(i) && !EntityList.IsDead(i)) {
				...
				if (isRage && !isTrigger && !isAimbot) {
					...
					if (fabs(boneScreen[0] - currentAng[0]) < 2.0f && fabs(boneScreen[1] - currentAng[1]) < 1.0f  && EntityList.GetTeam(i) != 0 && (isAimWall || bsp.Visible(myWorld, boneWorld)) && (isAimbotFriendly || imT != enT)) {
