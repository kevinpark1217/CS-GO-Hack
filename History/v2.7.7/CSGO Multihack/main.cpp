	...
	while (true) {
		if (isPanic)
			...
			if (i != -1 && NewPlayer.GetWeaponClip() > 0 && ((isHoldAim[activeMode] && GetAsyncKeyState(aimbotHoldKey) & 0x8000) || NewPlayer.GetShotsFired() > 0)) {
				...
				} while (fabs(boneScreen[0] - currentAng[0]) < aimHeight / 10.f && fabs(boneScreen[1] - currentAng[1]) < aimWidth / 10.f &&!EntityList.IsDead(i) && !EntityList.IsDormant(i) && (NewPlayer.GetWeaponClip() > 0 && ((isHoldAim[activeMode] && GetAsyncKeyState(aimbotHoldKey) & 0x8000) || NewPlayer.GetShotsFired() > 0) && (isAimWall || bsp.Visible(myWorld, boneWorld))));
			}
		}
		Sleep(V1SLEEP + 20);
	}
