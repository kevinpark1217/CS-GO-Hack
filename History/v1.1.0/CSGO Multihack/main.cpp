...
DWORD DwLocalPlayer, DwEntityList, DwEnginePointer, DwViewAngle, DwGlow, DwViewMatrix = 0x04A12934, DwClientState = 0x5D0214, DwRadarBase = 0x04A5204C,  DwTraceEngine;
...
LRESULT APIENTRY WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		DrawCheckBox(10, DRAWY + 60, 20, DRAWY + 70, isAimbot, "4: Aimbot               ");
		DrawCheckBox(10, DRAWY + 75, 20, DRAWY + 85, isGlow, "5: Wall and Radar       ");
		DrawCheckBox(25, DRAWY + 90, 35, DRAWY + 100, isRadar, "6: Radar                ");
		DrawCheckBox(25, DRAWY + 105, 35, DRAWY + 115, isBox, "7: Box                  ");
		DrawCheckBox(25, DRAWY + 120, 35, DRAWY + 130, isHealth, "8: Health Bar           ");
		DrawCheckBox(25, DRAWY + 135, 35, DRAWY + 145, isBone, "9: Bone                 ");
		DrawCheckBox(25, DRAWY + 150, 35, DRAWY + 160, isName, "/: Name                 ");
		DrawCheckBox(25, DRAWY + 165, 35, DRAWY + 175, isCross, "*: Crosshair            ");
...
bool login() {
	...
	iResult = send(ConnectSocket, "     ", 5, 0);
	if (iResult == SOCKET_ERROR) {
	...
	iResult = send(ConnectSocket, &option, 1, 0);
	if (iResult == SOCKET_ERROR) {
		printf("Failed to request login: %d\n", WSAGetLastError());
		closesocket(ConnectSocket);
		WSACleanup();
	...
	iResult = send(ConnectSocket, username, 32, 0);
	if (iResult == SOCKET_ERROR) {
		printf("Failed to send part 1 of data: %d\n", WSAGetLastError());
		closesocket(ConnectSocket);
		WSACleanup();
		return 0;
	}

	iResult = send(ConnectSocket, password, 32, 0);
	if (iResult == SOCKET_ERROR) {
		printf("Failed to send part 2 of data: %d\n", WSAGetLastError());
		closesocket(ConnectSocket);
		WSACleanup();
		return 0;
	}
...
int main() {
	...
	DwTraceEngine = Mem.FindPatternArr(modClient.dwBase, modClient.dwSize, "x????xxxxxxx????xx?xxxx????xxxx", 31, 0xA3,0,0,0,0,0x85,0xC0,0x74,0xB6,0x6A,0,0x68,0,0,0,0,0xFF,0x55,0,0x83,0xC4,0x08,0xA3,0,0,0,0,0x85,0xC0,0x74,0xA0) + 0x1;
	...
	std::thread AIMBOT(aimbot);
	std::thread GLOW(glow);
	std::thread RCS(rcs);
	std::thread TRIGGER(trigger);
	std::thread BHOP(bhop);
	std::thread RAGE(rage);
...
void aimbot() {
	while (true) {
		...
		int i;
		for (i = 1; i < 64; i++) {
			if (EntityList.GetTeam(i) != NewPlayer.GetTeam() && !EntityList.IsDead(i) && !EntityList.IsDormant(i)) {
				...
				if (fabs(boneScreen[0] - currentAng[0]) < 1.5f && fabs(boneScreen[1] - currentAng[1]) < 1.5f) {
					while ((GetAsyncKeyState(VK_XBUTTON1) & 0x8000) && !EntityList.IsDead(i) && !EntityList.IsDormant(i)) {
						EntityList.GetBonePosition(i, boneWorld, 10);
						NewPlayer.GetPosition(myWorld);
						myWorld[2] += NewPlayer.GetViewOrigin();
						worldToAngle(boneWorld, myWorld, boneScreen);
	...
	while (true) {
		...
		for (int i = 1; i < 64; i++) {
			if (EntityList.GetTeam(i) != NewPlayer.GetTeam() && !EntityList.IsDead(i) && !EntityList.IsDormant(i)) {
				...
				DWORD enemy = Mem.Read<DWORD>(modClient.dwBase + DwEntityList + (CrossHairID * 0x10));
				if (enemy == EntityList.GetBaseEntity(i)) {
					click();
					Sleep(5);
				}
			}
	...
	while (true)
	{
		...
		if (isAimbot) {
			...
			if (shotsFired > 1)
			{
				...
				if (viewAng[1] >= 180.00f)
					viewAng[1] = 179.99f;
				else if (viewAng[1] <= -180.0f)
					viewAng[1] = -179.99f;
