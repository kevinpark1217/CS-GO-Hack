...
struct
{
	...
	int GetPendingTeam(int PlayerNumber)
	{
		DWORD BaseEntity = GetBaseEntity(PlayerNumber);
		return Mem.Read<int>(BaseEntity + DwPendingTeamNumber);
	}
...
bool isTriggerToggle = false;
int triggerToggleKey = 97;
int triggerDelay = 1;
bool isTriggerFriendly = false;
bool isTriggerHold = false;
int triggerHoldKey = 5;

bool isRcs = false;
bool isRcsToggle = false;
int rcsToggleKey = 98;
int rcsAmount = 100;
bool isRcsCross = false;

bool isAimbot = false;
bool isAimbotToggle = false;
int aimbotToggleKey = 99;
int aimBone = 10;
bool isAimbotFriendly = false;
bool isLegitAim = true;
bool isAimWall = false;
int aimbotHoldKey = 5;

bool isBhop = false;
bool isBhopToggle = false;
int bhopToggleKey = 96;
int bhopGameBind = VK_SPACE;
int bhopJumpBind = VK_MENU;

bool isWall = false;
bool isWallToggle = false;
int wallToggleKey = 100;
...
bool isRadar = false;
bool isRadarToggle = false;
int radarToggleKey = 101;
bool isFlash = false;
bool isFlashToggle = false;
int flashToggleKey = 102;
bool isInfo = false;
bool isInfoToggle = false;
int infoToggleKey = 103;
int infoX = 10;
int infoY = 675;

bool isRage = false;
bool isRageToggle = false;
int rageToggleKey = 104;
bool isRageSpin = true;
float rageSpeed = 5.0f;
bool isRageFriendly = false;

int panelKey = VK_CAPITAL;
int panicKey = VK_HOME;
...
void loadConfig() {
	...
	if (config.is_open())
	{
		...
		while (getline(config, line)) {
			...
			else if (line.find("isLegitAim:") == 0)
				isLegitAim = (bool)stoi(line.substr(line.find(":") + 1));
			...
			else if (line.find("isFriendly:") == 0)
				isFriendly = (bool)stoi(line.substr(line.find(":") + 1));
			else if (line.find("isInternal:") == 0)
				isInternal = (bool)stoi(line.substr(line.find(":") + 1));
			else if (line.find("isExternal:") == 0)
				isExternal = (bool)stoi(line.substr(line.find(":") + 1));
			...
			else if (line.find("isRageSpin:") == 0)
				isRageSpin = (bool)stoi(line.substr(line.find(":") + 1));
			else if (line.find("rageSpeed:") == 0)
				rageSpeed = stof(line.substr(line.find(":") + 1));
...
void saveConfig() {
	...
	if (config.is_open())
	{
		...
		config << "isRageSpin:" << isRageSpin << "\n";
		config << "rageSpeed:" << rageSpeed << "\n";
...
#define PanelX 1024.0f
#define PanelY 768.0f
bool isExecuted[2][4];
bool isButton = false;
void DrawPanel() {
	float x = (float)(c_Rect.right - c_Rect.left)/2- PanelX/2;
	float y = (float)(c_Rect.bottom - c_Rect.top)/2- PanelY/2;
	...
	float xx = (PanelX - 30)/4;
	float yy = (PanelY - 90)/2;
	std::string topics[2][4] = { { "Auto Shoot", "Recoil Control", "Aimbot", "Bunny Hop" },{ "Wall","Miscellaneous","Rage","Setting" } };
	glLineWidth(1);
	for (int j = 0;j < 2;j++) {
		for (int i = 0;i < 4;i++) {
			glColor4f((GLfloat)1, (GLfloat)1, (GLfloat)1, 1.0f);
			...
			glColor4f((GLfloat)1, (GLfloat)1, (GLfloat)1, 1.0f);
			glRasterPos2f(x + xx*i + 30, y + yy*j + 60);
			glCallLists(strlen(topics[j][i].c_str()), GL_UNSIGNED_BYTE, topics[j][i].c_str());

			if (i == 0 && j == 0) {
				glRasterPos2f(x + xx*i + 30, y + yy*j + 100);
				glCallLists(8, GL_UNSIGNED_BYTE, "Enable");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 125);
				glCallLists(14, GL_UNSIGNED_BYTE, "Enable Toggle");
				if (!isTriggerToggle)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 150);
				glCallLists(11, GL_UNSIGNED_BYTE, "Toggle Key");
				if (!isTriggerToggle)
					glColor4f((GLfloat)1, (GLfloat)1, (GLfloat)1, 1.0f);
				if (!isTrigger)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 175);
				glCallLists(6, GL_UNSIGNED_BYTE, "Delay");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 200);
				glCallLists(14, GL_UNSIGNED_BYTE, "Friendly Fire");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 225);
				glCallLists(16, GL_UNSIGNED_BYTE, "Enable Hold Key");
				if (isTrigger && !isTriggerHold)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 250);
				glCallLists(9, GL_UNSIGNED_BYTE, "Hold Key");
				if (isTrigger && !isTriggerHold)
					glColor4f((GLfloat)1, (GLfloat)1, (GLfloat)1, 1.0f);
				//glEnd();

				POINT p;
				if (GetCursorPos(&p) && p.x > x + xx*i + 150 + 29 && p.x < x + xx*i + 150 + 34 && p.y > y + yy*j + 164 && p.y < y + yy*j + 170 && (GetKeyState(VK_LBUTTON) & 0x100) != 0)
				{
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150 + 29, y + yy*j + 164, 5, 6);
					if (!isExecuted[j][i] && triggerDelay < 999) {
						isExecuted[j][i] = true;
						triggerDelay++;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 + 29 && p.x < x + xx*i + 150 + 34 && p.y > y + yy*j + 171 && p.y < y + yy*j + 177 && (GetKeyState(VK_LBUTTON) & 0x100) != 0)
				{
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150 + 29, y + yy*j + 171, 5, 6);
					if (!isExecuted[j][i] && triggerDelay > 0) {
						isExecuted[j][i] = true;
						triggerDelay--;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 28 && p.y > y + yy*j + 139 && p.y < y + yy*j + 152 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150, y + yy*j + 139, 28, 13);
					if (!isExecuted[j][i]) {
						std::thread(&keyScan, &triggerToggleKey).detach();
						isExecuted[j][i] = true;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 88 && p.y < y + yy*j + 102 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isTrigger = !isTrigger;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 113 && p.y < y + yy*j + 127 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isTriggerToggle = !isTriggerToggle;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 188 && p.y < y + yy*j + 202 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isTriggerFriendly = !isTriggerFriendly;
					}
				} 
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 213 && p.y < y + yy*j + 227 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isTriggerHold = !isTriggerHold;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 29 && p.y > y + yy*j + 238 && p.y < y + yy*j + 252 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150, y + yy*j + 238, 29, 14);
					if (!isExecuted[j][i]) {
						std::thread(&keyScan, &triggerHoldKey).detach();
						isExecuted[j][i] = true;
					}
				}
				else
					isExecuted[j][i] = false;

				char buf[32];
				glColor4f((GLfloat)1, (GLfloat)1, (GLfloat)1, 1.0f);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 88, 14, 14); //Enable
				if (isTrigger)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 90, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 113, 14, 14); //TriggerToggle
				if (isTriggerToggle)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 115, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 138, 29, 14); //Toggle Key
				snprintf(buf, 32, "%3d", triggerToggleKey);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 150);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 163, 29, 14); //Delay
				snprintf(buf, 32, "%3d     ms", triggerDelay);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 175);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150 + 29, y + yy*j + 163, 6, 7); //Delay Up
				DrawPanelBox(x + xx*i + 150 + 29, y + yy*j + 170, 6, 7); //Delay Down
				DrawPanelBox(x + xx*i + 150, y + yy*j + 188, 14, 14); //Friendly Fire
				if (isTriggerFriendly)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 190, 11, 11); //Enable Friendly
				DrawPanelBox(x + xx*i + 150, y + yy*j + 213, 14, 14); //Enable Hold Key
				if (isTriggerHold)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 215, 11, 11); //Enable Hold Key
				DrawPanelBox(x + xx*i + 150, y + yy*j + 238, 29, 14); //Hold Key
				snprintf(buf, 32, "%3d", triggerHoldKey);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 250);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
			}
			else if (i == 1 && j == 0) {
				glRasterPos2f(x + xx*i + 30, y + yy*j + 100);
				...
				if (!isRcs)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 175);
				glCallLists(14, GL_UNSIGNED_BYTE, "Recoil Amount");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 200);
				glCallLists(10, GL_UNSIGNED_BYTE, "Crosshair");

				POINT p;
				if (GetCursorPos(&p) && p.x > x + xx*i + 150 + 29 && p.x < x + xx*i + 150 + 34 && p.y > y + yy*j + 164 && p.y < y + yy*j + 170 && (GetKeyState(VK_LBUTTON) & 0x100) != 0)
				{
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150 + 29, y + yy*j + 164, 5, 6); //Recoil Amount Up
					if (!isExecuted[j][i] && rcsAmount < 100) {
						isExecuted[j][i] = true;
						rcsAmount++;
					}
				}
				...
				{
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150 + 29, y + yy*j + 171, 5, 6); //Recoil Amount Down
					if (!isExecuted[j][i] && rcsAmount > 0) {
						isExecuted[j][i] = true;
						rcsAmount--;
					}
				}
					...
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150, y + yy*j + 139, 29, 14); //Rcs Key
					if (!isExecuted[j][i]) {
						std::thread(&keyScan, &rcsToggleKey).detach();
						isExecuted[j][i] = true;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 88 && p.y < y + yy*j + 102 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					...
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isRcsToggle = !isRcsToggle;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 188 && p.y < y + yy*j + 202 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isRcsCross = !isRcsCross;
					}
				}
				else
					isExecuted[j][i] = false;
				...
				DrawPanelBox(x + xx*i + 150, y + yy*j + 88, 14, 14); //Enable
				if (isRcs)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 90, 11, 11); //Enable
				DrawPanelBox(x + xx*i + 150, y + yy*j + 113, 14, 14); //Recoil Toggle
				if (isRcsToggle)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 115, 11, 11); //Enable Toggle
				DrawPanelBox(x + xx*i + 150, y + yy*j + 138, 29, 14); //Recoil Key
				snprintf(buf, 32, "%3d", rcsToggleKey);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 150);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 163, 29, 14); //Recoil Amount
				snprintf(buf, 32, "%3d     Percent", rcsAmount);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 175);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150 + 29, y + yy*j + 163, 6, 7); //Accuracy Up
				DrawPanelBox(x + xx*i + 150 + 29, y + yy*j + 170, 6, 7); //Accrracy Down
				DrawPanelBox(x + xx*i + 150, y + yy*j + 188, 14, 14);
				if (isRcsCross)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 190, 11, 11);
			}
			else if (i == 2 && j == 0) {
				glRasterPos2f(x + xx*i + 30, y + yy*j + 100);
				glCallLists(8, GL_UNSIGNED_BYTE, "Enable");
				...
				if (!isAimbot)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 175);
				glCallLists(13, GL_UNSIGNED_BYTE, "Aim Position");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 200);
				glCallLists(13, GL_UNSIGNED_BYTE, "Friendly Aim");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 225);
				glCallLists(13, GL_UNSIGNED_BYTE, "Legit Aimbot");
				if(isAimbot && isLegitAim)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 250);
				glCallLists(11, GL_UNSIGNED_BYTE, "Wall Trace");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 275);
				glCallLists(9, GL_UNSIGNED_BYTE, "Hold Key");
				if (isAimbot && isLegitAim)
					glColor4f((GLfloat)1, (GLfloat)1, (GLfloat)1, 1.0f);

				POINT p;
				if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 28 && p.y > y + yy*j + 139 && p.y < y + yy*j + 152 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150, y + yy*j + 139, 29, 14); //Rcs Key
					if (!isExecuted[j][i]) {
						std::thread(&keyScan, &aimbotToggleKey).detach();
						isExecuted[j][i] = true;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 88 && p.y < y + yy*j + 102 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					...
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isAimbotToggle = !isAimbotToggle;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 44 && p.y > y + yy*j + 163 && p.y < y + yy*j + 177 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150, y + yy*j + 163, 44, 14); //Aimbone
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						if (aimBone == 6)
							aimBone = 5;
					...
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isAimbotFriendly = !isAimbotFriendly;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 213 && p.y < y + yy*j + 227 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isLegitAim = !isLegitAim;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 238 && p.y < y + yy*j + 252 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isAimWall = !isAimWall;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 29 && p.y > y + yy*j + 263 && p.y < y + yy*j + 277 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150, y + yy*j + 263, 29, 14); //Hold Key
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						std::thread(&keyScan, &aimbotHoldKey).detach();
					}
				}
				else
					isExecuted[j][i] = false;
				...
				if (isAimbotToggle)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 115, 11, 11); //Enable Toggle
				DrawPanelBox(x + xx*i + 150, y + yy*j + 138, 29, 14); //Recoil Key
				snprintf(buf, 32, "%3d", aimbotToggleKey);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 150);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 163, 44, 14); //Aim Bone
				if(aimBone == 6) 
					snprintf(buf, 32, "Head", aimBone);
				else if (aimBone == 5)
					snprintf(buf, 32, "Neck", aimBone);
				else if (aimBone == 4)
					snprintf(buf, 32, "Chest", aimBone);
				else if (aimBone == 3)
					snprintf(buf, 32, "Belly", aimBone);
				else if (aimBone == 2)
					snprintf(buf, 32, "Waist", aimBone);
				else if (aimBone == 0)
					snprintf(buf, 32, "Hip", aimBone);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 175);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 188, 14, 14); //Friendly
				if(isAimbotFriendly)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 190, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 213, 14, 14); //Legit
				if(isLegitAim)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 215, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 238, 14, 14); //Wall
				if(isAimWall)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 240, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 263, 29, 14); //Hold Key
				snprintf(buf, 32, "%3d", aimbotHoldKey);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 275);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
			}
			else if (i == 3 && j == 0) {
				glRasterPos2f(x + xx*i + 30, y + yy*j + 100);
				...
				if (!isBhop)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 175);
				glCallLists(10, GL_UNSIGNED_BYTE, "Jump Bind");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 200);
				glCallLists(9, GL_UNSIGNED_BYTE, "BHop Key");
				//glEnd();
				...
				if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 28 && p.y > y + yy*j + 139 && p.y < y + yy*j + 152 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					...
					DrawPanelFill(x + xx*i + 150, y + yy*j + 139, 28, 13);
					if (!isExecuted[j][i]) {
						std::thread(&keyScan, &bhopToggleKey).detach();
						isExecuted[j][i] = true;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 88 && p.y < y + yy*j + 102 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					...
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isBhopToggle = !isBhopToggle;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 29 && p.y > y + yy*j + 163 && p.y < y + yy*j + 177 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150, y + yy*j + 163, 29, 14);
					if (!isExecuted[j][i]) {
						std::thread(&keyScan, &bhopGameBind).detach();
						isExecuted[j][i] = true;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 29 && p.y > y + yy*j + 188 && p.y < y + yy*j + 202 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150, y + yy*j + 188, 29, 14);
					if (!isExecuted[j][i]) {
						std::thread(&keyScan, &bhopJumpBind).detach();
						isExecuted[j][i] = true;
					}
				}
				else
				...
				if (isBhop)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 90, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 113, 14, 14); //BhopToggle
				if (isBhopToggle)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 115, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 138, 29, 14); //Toggle Key
				snprintf(buf, 32, "%3d", bhopToggleKey);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 150);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 163, 29, 14); //Game Bind Key
				snprintf(buf, 32, "%3d", bhopGameBind);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 175);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 188, 29, 14); //Game Bind Key
				snprintf(buf, 32, "%3d", bhopJumpBind);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 200);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
			}
			else if (i == 0 && j == 1) {
				glRasterPos2f(x + xx*i + 30, y + yy*j + 100);
				...
				if (!isWall)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 175);
				glCallLists(9, GL_UNSIGNED_BYTE, "Internal");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 200);
				glCallLists(9, GL_UNSIGNED_BYTE, "External");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 225);
				glCallLists(9, GL_UNSIGNED_BYTE, "Friendly");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 250);
				glCallLists(11, GL_UNSIGNED_BYTE, "Player Box");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 275);
				glCallLists(11, GL_UNSIGNED_BYTE, "Health Bar");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 300);
				glCallLists(12, GL_UNSIGNED_BYTE, "Player Name");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 325);
				glCallLists(14, GL_UNSIGNED_BYTE, "Player Weapon");
				...
				if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 28 && p.y > y + yy*j + 139 && p.y < y + yy*j + 152 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					...
					DrawPanelFill(x + xx*i + 150, y + yy*j + 139, 28, 13);
					if (!isExecuted[j][i]) {
						std::thread(&keyScan, &wallToggleKey).detach();
						isExecuted[j][i] = true;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 88 && p.y < y + yy*j + 102 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
				...
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 163 && p.y < y + yy*j + 177 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
				...
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 263 && p.y < y + yy*j + 277 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isHealth = !isHealth;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 288 && p.y < y + yy*j + 302 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isName= !isName;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 313 && p.y < y + yy*j + 327 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isWeapon = !isWeapon;
					}
				...
				if (isWall)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 90, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 113, 14, 14); //WallToggle
				if (isWallToggle)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 115, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 138, 29, 14); //Toggle Key
				snprintf(buf, 32, "%3d", wallToggleKey);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 150);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 163, 14, 14); //Internal
				if (isInternal)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 165, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 188, 14, 14); //External
				if (isExternal)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 190, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 213, 14, 14); //Frendly
				if (isFriendly)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 215, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 238, 14, 14); //Box
				if (isBox)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 240, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 263, 14, 14); //Health
				if (isHealth)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 265, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 288, 14, 14); //Name
				if (isName)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 290, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 313, 14, 14); //Weapon
				if (isWeapon)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 315, 11, 11);
			}
			else if (i == 1 && j == 1) {
				glRasterPos2f(x + xx*i + 30, y + yy*j + 100);
				glCallLists(6, GL_UNSIGNED_BYTE, "Radar");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 125);
				glCallLists(13, GL_UNSIGNED_BYTE, "Radar Toggle");
				if (!isRadarToggle)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 150);
				glCallLists(11, GL_UNSIGNED_BYTE, "Toggle Key");
				...
				glRasterPos2f(x + xx*i + 30, y + yy*j + 175);
				glCallLists(6, GL_UNSIGNED_BYTE, "Flash");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 200);
				glCallLists(13, GL_UNSIGNED_BYTE, "Flash Toggle");
				if (!isFlashToggle)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 225);
				glCallLists(11, GL_UNSIGNED_BYTE, "Toggle Key");
				...
				glRasterPos2f(x + xx*i + 30, y + yy*j + 250);
				glCallLists(5, GL_UNSIGNED_BYTE, "Info");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 275);
				glCallLists(12, GL_UNSIGNED_BYTE, "Info Toggle");
				if (!isInfoToggle)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 300);
				glCallLists(11, GL_UNSIGNED_BYTE, "Toggle Key");
				...
				if(!isInfo)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 325);
				glCallLists(13, GL_UNSIGNED_BYTE, "X Coordinate");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 350);
				glCallLists(13, GL_UNSIGNED_BYTE, "Y Coordinate");

				POINT p;
				if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 28 && p.y > y + yy*j + 138 && p.y < y + yy*j + 152 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150, y + yy*j + 139, 28, 13);
					if (!isExecuted[j][i]) {
						std::thread(&keyScan, &radarToggleKey).detach();
						isExecuted[j][i] = true;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 88 && p.y < y + yy*j + 102 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					...
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isRadarToggle = !isRadarToggle;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 28 && p.y > y + yy*j + 213 && p.y < y + yy*j + 227 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150, y + yy*j + 214, 28, 13);
					if (!isExecuted[j][i]) {
						std::thread(&keyScan, &flashToggleKey).detach();
						isExecuted[j][i] = true;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 163 && p.y < y + yy*j + 177 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
				...
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 28 && p.y > y + yy*j + 288 && p.y < y + yy*j + 302 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150, y + yy*j + 289, 28, 13);
					if (!isExecuted[j][i]) {
						std::thread(&keyScan, &infoToggleKey).detach();
						isExecuted[j][i] = true;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 238 && p.y < y + yy*j + 252 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
				...
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 + 29 && p.x < x + xx*i + 150 + 34 && p.y > y + yy*j + 314 && p.y < y + yy*j + 320 && (GetKeyState(VK_LBUTTON) & 0x100) != 0)
				{
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150 + 29, y + yy*j + 314, 5, 6);
					if (!isExecuted[j][i] && infoX < 999) {
						isExecuted[j][i] = true;
						infoX++;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 + 29 && p.x < x + xx*i + 150 + 34 && p.y > y + yy*j + 321 && p.y < y + yy*j + 327 && (GetKeyState(VK_LBUTTON) & 0x100) != 0)
				{
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150 + 29, y + yy*j + 321, 5, 6);
					if (!isExecuted[j][i] && infoX > 0) {
						isExecuted[j][i] = true;
						infoX--;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 + 29 && p.x < x + xx*i + 150 + 34 && p.y > y + yy*j + 339 && p.y < y + yy*j + 345 && (GetKeyState(VK_LBUTTON) & 0x100) != 0)
				{
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150 + 29, y + yy*j + 339, 5, 6);
					if (!isExecuted[j][i] && infoY < 999) {
						isExecuted[j][i] = true;
						infoY++;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 + 29 && p.x < x + xx*i + 150 + 34 && p.y > y + yy*j + 346 && p.y < y + yy*j + 352 && (GetKeyState(VK_LBUTTON) & 0x100) != 0)
				{
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150 + 29, y + yy*j + 346, 5, 6);
					if (!isExecuted[j][i] && infoY> 0) {
						isExecuted[j][i] = true;
						infoY--;
					}
				}
				...
				if (isRadar)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 90, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 113, 14, 14); //RadarToggle
				if (isRadarToggle)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 115, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 138, 29, 14); //Toggle Key
				snprintf(buf, 32, "%3d", radarToggleKey);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 150);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 163, 14, 14); //Enable
				if (isFlash)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 165, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 188, 14, 14); //FlashToggle
				if (isFlashToggle)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 190, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 213, 29, 14); //Toggle Key
				snprintf(buf, 32, "%3d", flashToggleKey);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 225);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 238, 14, 14); //Enable
				if (isInfo)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 240, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 263, 14, 14); //InfoToggle
				if (isInfoToggle)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 265, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 288, 29, 14); //Toggle Key
				snprintf(buf, 32, "%3d", infoToggleKey);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 300);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 313, 29, 14);
				snprintf(buf, 32, "%3d", infoX);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 325);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150 + 29, y + yy*j + 313, 6, 7); //Delay Up
				DrawPanelBox(x + xx*i + 150 + 29, y + yy*j + 320, 6, 7); //Delay Down
				DrawPanelBox(x + xx*i + 150, y + yy*j + 338, 29, 14);
				snprintf(buf, 32, "%3d", infoY);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 350);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150 + 29, y + yy*j + 338, 6, 7); //Delay Up
				DrawPanelBox(x + xx*i + 150 + 29, y + yy*j + 345, 6, 7); //Delay Down
			}
			else if (i == 2 && j == 1) {
				if(isAimbot || isTrigger)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 100);
				glCallLists(9, GL_UNSIGNED_BYTE, "Rage Bot");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 125);
				glCallLists(12, GL_UNSIGNED_BYTE, "Rage Toggle");
				if (!(isAimbot || isTrigger) && !isRageToggle)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 150);
				glCallLists(11, GL_UNSIGNED_BYTE, "Toggle Key");
				if (!(isAimbot || isTrigger) && !isRageToggle)
					glColor4f((GLfloat)1, (GLfloat)1, (GLfloat)1, 1.0f);
				if (!(isAimbot || isTrigger) && !isRage)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j + 175);
				glCallLists(9, GL_UNSIGNED_BYTE, "Spin Bot");
				if(!(isAimbot || isTrigger) && isRage && !isRageSpin)
					glColor4f((GLfloat)0.75f, (GLfloat)0.75f, (GLfloat)0.75f, 1.0f);
				glRasterPos2f(x + xx*i + 30, y + yy*j +200);
				glCallLists(11, GL_UNSIGNED_BYTE, "Spin Speed");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 225);
				glCallLists(9, GL_UNSIGNED_BYTE, "Friendly");
				...
				if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 28 && p.y > y + yy*j + 138 && p.y < y + yy*j + 152 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					...
					DrawPanelFill(x + xx*i + 150, y + yy*j + 139, 28, 13);
					if (!isExecuted[j][i]) {
						std::thread(&keyScan, &rageToggleKey).detach();
						isExecuted[j][i] = true;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 88 && p.y < y + yy*j + 102 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					...
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isRageToggle = !isRageToggle;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 163 && p.y < y + yy*j + 177 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isRageSpin = !isRageSpin;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 14 && p.y > y + yy*j + 213 && p.y < y + yy*j + 227 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					if (!isExecuted[j][i]) {
						isExecuted[j][i] = true;
						isRageFriendly = !isRageFriendly;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 + 29 && p.x < x + xx*i + 150 + 34 && p.y > y + yy*j + 189 && p.y < y + yy*j + 195 && (GetKeyState(VK_LBUTTON) & 0x100) != 0)
				{
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150 + 29, y + yy*j + 189, 5, 6);
					if (!isExecuted[j][i] && infoX < 999) {
						isExecuted[j][i] = true;
						rageSpeed+=0.1f;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 + 29 && p.x < x + xx*i + 150 + 34 && p.y > y + yy*j + 196 && p.y < y + yy*j + 202 && (GetKeyState(VK_LBUTTON) & 0x100) != 0)
				{
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150 + 29, y + yy*j + 196, 5, 6);
					if (!isExecuted[j][i] && infoX > 0) {
						isExecuted[j][i] = true;
						rageSpeed -= 0.1f;
					}
				...
				if (isRageToggle)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 115, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 138, 29, 14); //Toggle Key
				snprintf(buf, 32, "%3d", rageToggleKey);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 150);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 163, 14, 14); //isSpin
				if (isRageSpin)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 165, 11, 11);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 188, 29, 14); //Spin Speed
				snprintf(buf, 32, "%1.1f    degrees", rageSpeed);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 200);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150 + 29, y + yy*j + 188, 6, 7); //Delay Up
				DrawPanelBox(x + xx*i + 150 + 29, y + yy*j + 195, 6, 7); //Delay Down
				DrawPanelBox(x + xx*i + 150, y + yy*j + 213, 14, 14); //Rage Friendly
				if (isRageFriendly)
					DrawPanelFill(x + xx*i + 151, y + yy*j + 215, 11, 11);
			}
			else if (i == 3 && j == 1) {
				glRasterPos2f(x + xx*i + 30, y + yy*j + 100);
				glCallLists(10, GL_UNSIGNED_BYTE, "Panel Key");
				glRasterPos2f(x + xx*i + 30, y + yy*j + 125);
				glCallLists(10, GL_UNSIGNED_BYTE, "Panic Key");

				POINT p;
				if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 28 && p.y > y + yy*j + 88 && p.y < y + yy*j + 102 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150, y + yy*j + 89, 28, 13);
					if (!isExecuted[j][i]) {
						std::thread(&keyScan, &panelKey).detach();
						isExecuted[j][i] = true;
					}
				}
				else if (GetCursorPos(&p) && p.x > x + xx*i + 150 && p.x < x + xx*i + 150 + 28 && p.y > y + yy*j + 113 && p.y < y + yy*j + 127 && (GetKeyState(VK_LBUTTON) & 0x100) != 0) {
					glColor4f((GLfloat)color0[0] / 255, (GLfloat)color0[1] / 255, (GLfloat)color0[2] / 255, 1.0f);
					DrawPanelFill(x + xx*i + 150, y + yy*j + 114, 28, 13);
					if (!isExecuted[j][i]) {
						std::thread(&keyScan, &panicKey).detach();
						isExecuted[j][i] = true;
					}
				}
				else
				...
				char buf[32];
				glColor4f((GLfloat)1, (GLfloat)1, (GLfloat)1, 1.0f);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 88, 29, 14); //Panel Key
				snprintf(buf, 32, "%3d", panelKey);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 100);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
				DrawPanelBox(x + xx*i + 150, y + yy*j + 113, 29, 14); //Panic Key
				snprintf(buf, 32, "%3d", panicKey);
				glRasterPos2f(x + xx*i + 152, y + yy*j + 125);
				glCallLists(strlen(buf), GL_UNSIGNED_BYTE, buf);
			}
		}
	}
	glLineWidth(2);
	std::string buttons[4] = { "Save Configuration", "Load Previous Save", "Reset to Default", "Exit" };
	for (int i = 0; i < 4; i++) {
						...
						for (int j = 1; j < 64; j++) {
							if (glowObj.pEntity == EntityList.GetBaseEntity(j) && glowObj.pEntity != NewPlayer.GetDwLocalPlayer()) {
								...
								if (!EntityList.IsDead(j) && !EntityList.IsDormant(j)) {
									...
									int color[4];
									if ((!isFriendly && EntityList.GetTeam(j) == NewPlayer.GetTeam()) && EntityList.GetTeam(i) != 0) {
										color[0] = 0;
										color[1] = 0;
										color[2] = 255;
									}
									...
									float boxPos[64][4];
									char chModel[64];
									DWORD model = Mem.Read<DWORD>(EntityList.GetBaseEntity(j) + 0x6C);
									for (int i = 0;i < 64;i++)
									...
									for (int p = 0;p < 4;p++) {
										if (p >= 2)
											...
											boxPos[j - 1][p] = std::numeric_limits<float>::max();
									}
									...
									for (int b = 0; b < boneCount; b++)
									{
										...
										if (isWall && isExternal)
											DrawSnaplines(bone2D[b][0], bone2D[b][1], bone2D[parentId][0], bone2D[parentId][1]);

										if (bone2D[b][0] < boxPos[j - 1][0] && bone2D[b][0]>0.0f)
											boxPos[j - 1][0] = bone2D[b][0];
										else if (bone2D[parentId][0] < boxPos[j - 1][0] && bone2D[parentId][0]>0.0f)
											boxPos[j - 1][0] = bone2D[parentId][0];

										if (bone2D[b][0] > boxPos[j - 1][2])
											boxPos[j - 1][2] = bone2D[b][0];
										else if (bone2D[parentId][0] > boxPos[j - 1][2])
											boxPos[j - 1][2] = bone2D[parentId][0];

										if (bone2D[b][1] < boxPos[j - 1][1] && bone2D[b][1]>0.0f)
											boxPos[j - 1][1] = bone2D[b][1];
										else if (bone2D[parentId][1] < boxPos[j - 1][1] && bone2D[parentId][1]>0.0f)
											boxPos[j - 1][1] = bone2D[parentId][1];

										if (bone2D[b][1] > boxPos[j - 1][3])
											boxPos[j - 1][3] = bone2D[b][1];
										else if (bone2D[parentId][1] > boxPos[j - 1][3])
											boxPos[j - 1][3] = bone2D[parentId][1];
									}
									if ((isFriendly || EntityList.GetTeam(j) != NewPlayer.GetTeam()) && EntityList.GetTeam(i) != 0) {
										float PlayerPos[3];
										...
										if (WorldToScreen(EnemyPos, EnemyXY))
										{
											...
											float HealthBarWidth = (boxPos[j - 1][2] - boxPos[j - 1][0]) / 5;
											float HealthBarHeight = (boxPos[j - 1][3] - boxPos[j - 1][1]) / 5;
											if (isBox)
												DrawBox(boxPos[j - 1][0] - HealthBarWidth, boxPos[j - 1][1] - HealthBarHeight, boxPos[j - 1][2] + HealthBarWidth, boxPos[j - 1][3] + HealthBarHeight, BoxColor);
											if (isHealth) {
												int HealthBarBackColor[3] = { 254, 0, 0 };
												int HealthBarColor[3] = { 0, 254, 0 };
												float HealthHeight = ((boxPos[j - 1][3] + HealthBarHeight - 1.0f) - (boxPos[j - 1][1] - HealthBarHeight + 1.0f))*((100.0f - EntityList.GetHealth(j)) / 100.0f);
												DrawBox(boxPos[j - 1][2] + HealthBarWidth, boxPos[j - 1][3] + HealthBarHeight, boxPos[j - 1][2] + HealthBarWidth*2, boxPos[j - 1][1] - HealthBarHeight, BoxColor);
												DrawFilledBox(boxPos[j - 1][2] + HealthBarWidth + 1.0f, boxPos[j - 1][3] + HealthBarHeight - 1.0f, boxPos[j - 1][2] + HealthBarWidth * 2 - 1.0f, boxPos[j - 1][1] - HealthBarHeight + 1.0f, HealthBarBackColor);
												DrawFilledBox(boxPos[j - 1][2] + HealthBarWidth + 1.0f, boxPos[j - 1][3] + HealthBarHeight - 1.0f, boxPos[j - 1][2] + HealthBarWidth * 2 - 1.0f, (boxPos[j - 1][1] - HealthBarHeight + 1.0f) + (HealthHeight), HealthBarColor);
											}
											...
											if (isName) {
												...
												glColor3f(204, 204, 0);
												glRasterPos2f(boxPos[j - 1][0] - HealthBarWidth, boxPos[j - 1][1] - HealthBarHeight - 4);
											...
											if (isWeapon) {
												...
												glColor3f(204, 204, 0);
												glRasterPos2f(boxPos[j - 1][0] - HealthBarWidth, boxPos[j - 1][3] + HealthBarHeight + 12);
...
#define SELF_REMOVE_STRING  TEXT("cmd.exe /C ping 1.1.1.1 -n 1 -w 5000 > Del \"%s\"")
...
#define SELF_REMOVEUPDATE_STRING  TEXT("cmd.exe /C ping 1.1.1.1 -n 1 -w 5000 > Del old.exe")
...
int main() {
	...
	//std::cout << "0x" << std::hex << DwEnginePointer << std::endl;
	...
	while (!isPanic) {
		...
		if (GetAsyncKeyState(panicKey) & 0x8000)
		{
			...
			isPanic = true;
			while (GetAsyncKeyState(panicKey) & 0x8000)
				Sleep(1);
			exit(1337);
		}
		...
		if(isInfo && GetAsyncKeyState(infoToggleKey) & 0x8000)
		{
			isInfo = !isInfo;
			while (GetAsyncKeyState(infoToggleKey) & 0x8000)
				Sleep(1);
		}
		if (isFlash && GetAsyncKeyState(flashToggleKey) & 0x8000)
		{
			isFlash = !isFlash;
...
void v0() {
	while (true) {
		if (isPanic)
			break;
		if (!isRunning || !isBhop && !isFlash && !isTrigger) {
			Sleep(250);
			continue;
		}
		if (isFlash) {
		...
		if (isTrigger) {
			...
			if ((!isTriggerHold || GetAsyncKeyState(triggerHoldKey) & 0x8000) && (isTriggerFriendly || MyTeam != EnemyTeam) && EnemyHealth > 0 && EnemyHealth <= 100)
			{
				Sleep(triggerDelay);
				click();
				Sleep(8);
		...
		for (int i = 1; i < 64; i++) {
			...
			if (!EntityList.IsDormant(i) && !EntityList.IsDead(i)) {
				...
				if (isRage && !isTrigger && !isAimbot) {
					if (bsp.Visible(myWorld, boneWorld) && (EntityList.GetTeam(i) != NewPlayer.GetTeam() || isRageFriendly) && !EntityList.IsDormant(i) && !EntityList.IsDead(i) && EntityList.GetTeam(i) != 0) {
						if (isRageSpin) {
							float angle[3];
							NewPlayer.GetAngles(angle);
							int loopCount = (int)((boneScreen[1] - angle[1]) / rageSpeed);
							float ychange = (boneScreen[0] - angle[0]) / loopCount;
							for (int i = 0;i < loopCount; i++) {
								angle[1] += rageSpeed;
								angle[0] += ychange;
								NewPlayer.SetAngles(angle);
								Sleep(5);
							}
							EntityList.GetBonePosition(i, boneWorld, 6);
							NewPlayer.GetPosition(myWorld);
						...
						Sleep(5);
						while (bsp.Visible(myWorld, boneWorld) && EntityList.GetBaseEntity(i) == EntityList.GetBaseEntity(NewPlayer.GetCrosshairId()) && isRage && !EntityList.IsDormant(i) && !EntityList.IsDead(i)) {
							click();
							Sleep(8);
							EntityList.GetBonePosition(i, boneWorld, 6);
							NewPlayer.GetPosition(myWorld);
					...
					if (fabs(boneScreen[0] - currentAng[0]) < 2.0f && fabs(boneScreen[1] - currentAng[1]) < 1.0f && (isAimWall || bsp.Visible(myWorld, boneWorld)) && (isAimbotFriendly || EntityList.GetTeam(i) != NewPlayer.GetTeam()) && EntityList.GetTeam(i) != 0) {
						float val0 = (boneScreen[0] - currentAng[0])/5, val1 = (boneScreen[1] - currentAng[1])/5;
						int aim = 4;
						while ((isLegitAim || GetAsyncKeyState(aimbotHoldKey) & 0x8000) && (!isLegitAim || NewPlayer.GetShotsFired() > 0) && !EntityList.IsDead(i) && !EntityList.IsDormant(i) && (isAimWall || bsp.Visible(myWorld, boneWorld))) {
							if (fabs(val0/5) > 5.0f || fabs(val1/5) > 2.5f) {
								NewPlayer.GetAngles(currentAng);
		...
		if(isRadar && (!isWall || !isInternal))
			...
			if (pointerGlow != NULL) {
				for (int i = 0; i < objCount; i++) {
					...
					GlowObjectDefinition_t glowObj = Mem.ReadNew<GlowObjectDefinition_t>(mObj);
					if (glowObj.pEntity != NULL) {
						//std::cout << GlowList.GetIName(glowObj.pEntity) << " " << GlowList.GetTeam(glowObj.pEntity) << std::endl;
						for (int j = 1; j < 64; j++) {
							if (/*GlowList.GetIName(glowObj.pEntity)==0 || */glowObj.pEntity == EntityList.GetBaseEntity(j)) {
								float me3[4];
								float en3[4];
								EntityList.GetBonePosition(j, en3, 6);
								NewPlayer.GetPosition(me3);
