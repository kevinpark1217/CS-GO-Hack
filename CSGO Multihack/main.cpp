	...
	while (true) {
		if (isPanic)
			...
			for (loop = 0; loop < maxPlayer; loop++) {
				if (EntityList.GetBaseEntity(loop) == NULL || EntityList.GetBaseEntity(loop) == NewPlayer.GetDwLocalPlayer() || EntityList.IsDormant(loop) || EntityList.IsDead(loop) || (NewPlayer.GetTeam() == EntityList.GetTeam(loop) && !isAimbotFriendly))
					continue;
